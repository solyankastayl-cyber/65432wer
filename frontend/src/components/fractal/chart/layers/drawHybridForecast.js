...
export function drawMacroForecast(
  ...
  const lastHybridPrice = hybridData.length > 0 ? hybridData[hybridData.length - 1].price : null;
  const forecastEndPrice = lastMacroPrice || lastHybridPrice || anchorPrice;
  const isGoingUp = forecastEndPrice > anchorPrice;
  
  const legendX = xRightAnchor + 12;
  // If going up, put legend at bottom; if going down, put at top
  const legendY = isGoingUp 
